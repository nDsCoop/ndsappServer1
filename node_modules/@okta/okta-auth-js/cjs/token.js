...
function getOAuthParamsStrFromStorage() {
  ...
  let oauthParamsStr = cookies.get(_constants.REDIRECT_OAUTH_PARAMS_NAME);
  cookies.delete(_constants.REDIRECT_OAUTH_PARAMS_NAME); // latest version of auth-js will store params in session storage

  if (!oauthParamsStr && _browserStorage.default.browserHasSessionStorage()) {
    const storage = _browserStorage.default.getSessionStorage();

    oauthParamsStr = storage.getItem(_constants.REDIRECT_OAUTH_PARAMS_NAME);
    storage.removeItem(_constants.REDIRECT_OAUTH_PARAMS_NAME);
  }
